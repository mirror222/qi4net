using System;
using System.Text;
//http://www.cnblogs.com/skynice/archive/2008/11/26/longsmspdu.html
namespace Qi.Sms.Protocol.Encodes
{
    /// <summary>
    /// 针对国内短信编码（USC2）
    /// </summary>
    public class ChinaCmppPduEncoding
    {
        public static readonly int MAX_CHAR_COUNT = 70; //最长可发送汉字个数
        ...
        public static string EncodingSMS(string center, string phone, string content, out string length)
        {
            ...
            string result = String.Format("{0}11000D91{1}000800{2}", center, DecodingPhone(phone),
                                          EncodeingAddLength(content));
        ...
        /// <returns></returns>
        public static string EncodingSMS(string center, string phone, string content, int count, int i,
                                         out string length)
        {
            if (content.Length <= MAX_CHAR_COUNT)
            {
                return EncodingSMS(center, phone, content, out length);
            }
            else
            {
                if (count - 1 == i)
                {
                    content = content.Substring(i*(MAX_CHAR_COUNT - 6));
                }
                else
                {
                    content = content.Substring(i*(MAX_CHAR_COUNT - 6), MAX_CHAR_COUNT - 6);
                }
        ...
        /// <param name="length"></param>
        /// <returns></returns>
        public static string EncodingSMS(string center, string phone, int count, int i, string content,
                                         out string length)
        {
            string message = string.Empty;
            int msgcount = GetMaxEncodeCharCount()*2; //固定
